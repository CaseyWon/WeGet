...
function resourceName(type, index, url) {
  const extension = extensionFromUrl(url, type === 'video' ? 'mp4' : type === 'audio' ? 'mp3' : type === 'file' ? 'bin' : 'jpg');
  return `${TYPE_LABELS[type]}-${String(index).padStart(2, '0')}.${extension}`;
}
...
export function parseWeChatArticle(html, sourceUrl) {
  ...
  const title = cleanText(
    ...
    $('#activity-name').text() || scriptValue(html, 'msg_title') || $('title').text()
  ...
  const add = (rawUrl, type, extra = {}) => {
    ...
    counters[type] += 1;
    const resource = {
      id: resourceId(url, type),
      type,
      typeLabel: TYPE_LABELS[type],
      url,
      name: resourceName(type, counters[type], url),
      alt: cleanText(extra.alt || ''),
      width: Number(extra.width) || null,
      height: Number(extra.height) || null,
      source: extra.source || ''
    };
  ...
  const headCandidates = [
    ...
    scriptValue(html, 'msg_cdn_url'),
    scriptValue(html, 'cdn_url')
  ];
  headCandidates.forEach(url => add(url, 'head', { source: '文章头图' }));
  ...
  $('img').each((_, element) => {
    ...
    let type = 'image';
    if (looksLikeCard($, element)) type = 'card';
    const classes = `${node.attr('class') || ''} ${node.attr('id') || ''}`;
    if (/cover|cdn_url|rich_media_thumb/i.test(classes)) type = 'head';
    add(rawUrl, type, {
      alt: node.attr('alt') || node.attr('data-title'),
      width: node.attr('data-w') || node.attr('width'),
      height: node.attr('data-h') || node.attr('height'),
  ...
  $('video, iframe, mpvideo, mp-video').each((_, element) => {
    ...
    for (const attr of ['data-src', 'src', 'data-url', 'data-video-url']) {
  ...
  $('a[href]').each((_, element) => {
    ...
    if (FILE_EXTENSIONS.has(extension)) add(href, 'file', { alt: $(element).text(), source: '附件链接' });
  });
